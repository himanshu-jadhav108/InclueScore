-- Safe Sample Data for Project Zenith
-- This version handles conflicts and can be run multiple times safely

-- Insert sample tenants (with conflict handling)
...
-- Insert sample users with different roles (with conflict handling)
INSERT INTO users (id, clerk_user_id, email, first_name, last_name, role, phone, tenant_id) VALUES 
    -- Admin user (update existing admin or create new one)
...
-- Insert sample beneficiaries (with conflict handling)
...
ON CONFLICT (beneficiary_code) DO UPDATE SET 
    monthly_income = EXCLUDED.monthly_income,
    employment_type = EXCLUDED.employment_type,
    updated_at = CURRENT_TIMESTAMP;

-- Insert sample loan applications (with conflict handling)
...
SELECT 'Beneficiaries: ' || COUNT(*) as count FROM beneficiaries;
SELECT 'Loan Applications: ' || COUNT(*) as count FROM loan_applications;
SELECT 'Score History Records: ' || COUNT(*) as count FROM score_history;
